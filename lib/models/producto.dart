...
enum EstadoProducto { activo, descontinuado, agotado, estacional, oculto }

/// Géneros disponibles para productos de ropa
enum GeneroProducto { hombre, mujer, unisex, nino, nina }

/// Temporadas del año
enum TemporadaProducto { verano, invierno, primavera, otono, todoElAno }

// @Collection()
class Producto {
  int id = 0; // Cambiado de Id a int y inicializado
  ...
  Producto({
    this.id = 0, // Cambiado de Isar.autoIncrement
    this.nombre,
    this.descripcion,
    this.precio,
    this.stockActual,
  ...
  String? get codigo => codigoBarras;
  String? get imagenUrl => imagenPrincipal;
  ...
  factory Producto.fromJson(Map<String, dynamic> json) {
    final producto = Producto(
      ...
      genero:
          json['genero'] != null
              ? GeneroProducto.values.firstWhere(
                (e) => e.name == json['genero'],
              )
              : null,
      temporada:
          json['temporada'] != null
              ? TemporadaProducto.values.firstWhere(
  ...
  // Función auxiliar para parsear el ID de forma segura
  static int _parseId(dynamic value) {
    if (value == null) return 0; // Cambiado de Isar.autoIncrement
    if (value is int) return value;
    if (value is String) {
      final parsed = int.tryParse(value);
      return parsed ?? 0; // Cambiado de Isar.autoIncrement
    }
    return 0; // Cambiado de Isar.autoIncrement
  }
  ...
  bool get tieneStockBajo {
    if (stockMinimo == null || stockActual == null) return false;
  ...
  double get porcentajeStock {
    ...
    return ((stockActual ?? 0) / stockMaximo!) * 100;
  }
  ...
  void incrementarStock(int cantidad, [int? usuarioId]) {
    actualizarStock((stockActual ?? 0) + cantidad, usuarioId);
  }

  /// Decrementa el stock (por ventas)
  void decrementarStock(int cantidad, [int? usuarioId]) {
    final nuevoStock = (stockActual ?? 0) - cantidad;
